...
``[Slab].add_atom(id, el, x, y, z, uh = 0, uk = 0, ul = 0, oc = 1.0, m = 1.0)``
...
``uh``
    The mean-square displacement for the atom along h
``uk``
    The mean-square displacement for the atom along k
``ul``
    The mean-square displacement for the atom along l
``oc``
...
if USE_NUMBA:
    try:
        from .lib.surface_scattering import surface_lattice_sum_uhkl
    except ImportError:
        numba_ss=False
    else:
        numba_ss=True
...
class Sample:
    ...
    # Deltas for pseudocells added JES 1/5/2016
    ...
    def calc_fs(self, h, k, l):
        ...
        dhinv = self.unit_cell.abs_h(h)
        dkinv = self.unit_cell.abs_k(k)
        dlinv = self.unit_cell.abs_l(l)
        x, y, z, uh, uk, ul, oc, el = self._surf_pars()
        # print x, y,z
        # Create all the atomic structure factors
        f = self._get_f(el, dinv)
        # print f.shape, h.shape, oc.shape, x.shape, y.shape, z.shape
        # Deltas for pseudocells added 1/5/2016
        # Anisotropic disorder factors added 8/31/2016
        fs = np.sum(
            oc*f*np.exp(-2*np.pi**2*(uh*dhinv[:, np.newaxis]**2+
                                     uk*dkinv[:, np.newaxis]**2+
                                     ul*dlinv[:, np.newaxis]**2))
    ...
    def turbo_calc_fs(self, h, k, l):
        ...
        '''
        # turbo_calc DOES NOT WORK for structures with displacement vectors!
        ...
        fs = surface_lattice_sum_uhkl(x, y, z, h, k, l, uh, uk, ul, 0., oc, f, Pt, dinv)
        return fs
    ...
    def calc_fb(self, h, k, l):
        ...
        dkinv = self.unit_cell.abs_k(k)
        dlinv = self.unit_cell.abs_l(l)
        x, y, z, el, uh, uk, ul, oc, c = self.bulk_slab._extract_values()
        ...
        denom = 1.0-np.exp(-2.0*np.pi*1.0J*(h+k+l))*np.exp(-alpha)
    ...
    def calc_rhos(self, x, y, z, sb=0.8):
        ...
        '''
        # May be broken after switching to anisotropic disorder parameters...
        px, py, pz, uh, uk, ul, oc, el = self._surf_pars()
        ...
        rho = np.sum([np.sum([rho(self.unit_cell.dist(x, y, z,
                              ...
                              zip(rhos, px, py, pz, uh, uk, ul, oc)], 0)
                      for sym_op in self.surface_sym], 0)
        return rho
    ...
    def _surf_pars(self):
        ...
        xt, yt, zt, elt, uht, ukt, ult, oct, ct = zip(*[slab._extract_values()
        ...
        x = np.concatenate([xs+c_cum
                            for xs, c_cum, c_s in zip(xt, cn, ct)])
        y = np.concatenate([ys+c_cum
    ...
    def create_uc_output(self):
        ...
        for sym_op in self.surface_sym:
            ...
            zout = np.r_[zout, z]
            uhout = np.r_[uhout, uh]
            ukout = np.r_[ukout, uk]
            ulout = np.r_[ulout, ul]
            ocout = np.r_[ocout, oc]
            elout = np.r_[elout, el]
        idsout.extend(ids)

        return xout, yout, zout, uhout, ukout, ulout, ocout, elout, idsout
    ...
    def export_xyz(self, fname):
        ...
        if not fname.endswith('.xyz'):
            fname+='.xyz'
        x, y, z, uh, uk, ul, oc, el, ids = self.create_uc_output()
...
class UnitCell:
    ...
    def abs_h(self, h):
        '''Returns the absolute value of (h,0,0) vector in units of
        AA.

        This is equal to the inverse lattice spacing 1/d_h.
        '''
        dhinv = np.sqrt((h/self.a*np.sin(self.alpha))**2
                        /(1-np.cos(self.alpha)**2-np.cos(self.beta)**2
                          -np.cos(self.gamma)**2+2*np.cos(self.alpha)
                          *np.cos(self.beta)*np.cos(self.gamma)))
        return dhinv
    ...
    def abs_k(self, k):
        '''Returns the absolute value of (0,k,0) vector in units of
        AA.

        This is equal to the inverse lattice spacing 1/d_k.
        '''
        dkinv = np.sqrt((k/self.b*np.sin(self.beta))**2
                        /(1-np.cos(self.alpha)**2-np.cos(self.beta)**2
                          -np.cos(self.gamma)**2+2*np.cos(self.alpha)
                          *np.cos(self.beta)*np.cos(self.gamma)))
        return dkinv
    ...
    def abs_l(self, l):
        '''Returns the absolute value of (0,0,l) vector in units of
        AA.

        This is equal to the inverse lattice spacing 1/d_l.
        '''
        dlinv = np.sqrt((l/self.c*np.sin(self.gamma))**2
                        /(1-np.cos(self.alpha)**2-np.cos(self.beta)**2
                          -np.cos(self.gamma)**2+2*np.cos(self.alpha)
                          *np.cos(self.beta)*np.cos(self.gamma)))
        return dlinv
...
class Slab:
    ...
    def __init__(self, name='', c=1.0, slab_oc=1.0):
        ...
        self.uh = np.array([], dtype=np.float64)
        self.uk = np.array([], dtype=np.float64)
        self.ul = np.array([], dtype=np.float64)
    ...
    def __str__(self):
        ...
        for i in range(len(self.id)):
            ...
            out+=f'uh={self.uh[i]}, uk={self.uk[i]}, ul={self.ul[i]}, '
            out+=f'oc={self.oc[i]}, m={self.m[i]}'
        return out

    def add_atom(self, id, element, x, y, z, uh=0.0, uk=0.0, ul=0.0, oc=1.0, m=1.0):
        ...
        uh, uk, ul - anisotropic debye-waller parameters for the atom
        ...
        self.uh = np.append(self.uh, uh)
        self.uk = np.append(self.uk, uk)
        self.ul = np.append(self.ul, ul)
    ...
    def find_atoms(self, expression):
        ...
        if (type(expression)==type(np.array([])) or
            ...
            choose_list = [eval(expression) for x, y, z, uh, uk, ul, oc, el, id in
                           zip(self.x, self.y, self.z, self.uh, self.uk,
                               self.ul, self.oc, self.el, self.id)]
            # print choose_list
            ag = AtomGroup()
    ...
    def _extract_values(self):
        return self.x+self.dx, self.y+self.dy, self.z+self.dz, \
               self.el, self.uh, self.uk, self.ul, self.oc*self.m*self.slab_oc, self.c
...
class AtomGroup:
    par_names = ['dx', 'dy', 'dz', 'uh', 'uk', 'ul', 'oc']
...
# =============================================================================
# The examples below contain isotropic structure factors...
